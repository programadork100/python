...
class Productos:
    ...
    def __init__(self, root):
        ...
        btn2 = Button(frame2, text="Actualizar", command=self.Actualizar, width=12, height=2)
        btn2.grid(row=5, column=1, padx=10, pady=10)

        btn3 = Button(frame2, text="Eliminar", command=self.Eliminar, width=12, height=2)
    ...
    def Eliminar(self):
        ...
        query = 'DELETE FROM articulo WHERE nombre = ?'
    ...
    def edit_record(self, nue_vo, precio, nueva, cantidad):
        query = 'UPDATE articulo SET precio = ?, cantidad = ? WHERE precio = ? AND cantidad = ?'
    ...
    def consulta(self):
        ...
        query = "SELECT id, nombre, precio, cantidad FROM articulo" 
